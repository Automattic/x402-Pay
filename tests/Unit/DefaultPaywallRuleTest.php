...
final class DefaultPaywallRuleTest extends TestCase {
	...
	private function set_options( array $overrides ): void {
		...
		$GLOBALS['__x402_pay_options'][ SettingsRepository::OPTION_NAME ] = array_merge(
			array(
				'wallet_address'           => '',
				'default_price'            => '0.01',
	...
	public function test_category_mode_gates_post_in_bound_category(): void {
		$this->set_options( array() );
		$GLOBALS['__x402_pay_terms'] = array( array( self::DEFAULT_TERM_ID, 'category', 7 ) );
		$rule                     = $this->make_rule();
		$this->assertSame(
	...
	public function test_category_mode_uses_configured_term_id(): void {
		...
		$GLOBALS['__x402_pay_terms'] = array( array( 42, 'category', 7 ) );
		$rule                     = $this->make_rule();
	...
	public function test_category_mode_ignores_post_tag(): void {
		...
		$this->set_options( array() );
		$GLOBALS['__x402_pay_terms'] = array( array( self::DEFAULT_TERM_ID, 'post_tag', 7 ) );
		$rule                     = $this->make_rule();
	...
	public function test_preserves_rule_from_higher_priority_filter(): void {
		$this->set_options( array() );
		$GLOBALS['__x402_pay_terms'] = array( array( self::DEFAULT_TERM_ID, 'category', 42 ) );
		$rule                     = $this->make_rule();
	...
	public function test_all_posts_mode_gates_published_post(): void {
		...
		$GLOBALS['__x402_pay_posts'][ 99 ] = array(
			'post_type'   => 'post',
			'post_status' => 'publish',
		);
		$rule = $this->make_rule();
