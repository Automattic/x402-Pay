...
final class PaywallCategoryGuardTest extends TestCase {
	 ...
	 * gone from __x402_pay_existing_terms by the time the hook runs.
	 */
	...
	public function test_resets_binding_to_default_when_bound_term_is_deleted(): void {
		...
		$stored = $GLOBALS['__x402_pay_options'][ SettingsRepository::OPTION_NAME ];
		// Default term was freshly inserted; the guard bound to it.
		$this->assertCount( 1, $GLOBALS['__x402_pay_inserted_terms'] );
		$default_id = $GLOBALS['__x402_pay_inserted_terms'][0]['term_id'];
	...
	public function test_recreates_default_term_when_default_itself_is_deleted(): void {
		...
		$new_default_id = $GLOBALS['__x402_pay_inserted_terms'][0]['term_id'];
		$this->assertSame(
			$new_default_id,
			$GLOBALS['__x402_pay_options'][ SettingsRepository::OPTION_NAME ]['paywall_category_term_id']
		);
	}
	...
	public function test_emits_warning_notice_with_deleted_term_name(): void {
		...
		$this->assertContains( 'x402_pay_category_deleted', $codes );
		$messages = array_column( $GLOBALS['__x402_pay_settings_errors'], 'message' );
		$this->assertTrue(
