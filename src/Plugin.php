	...
	public static function boot(): void {
		...
		add_action(
			...
			static function () use ( $controller ): void {
				...
				foreach ( $GLOBALS['__x402_pay_response']['success_headers'] ?? array() as $line ) {
					header( (string) $line, false );
				}

				if ( ! empty( $GLOBALS['__x402_pay_response']['exited'] ) ) {
					foreach ( $GLOBALS['__x402_pay_response']['headers'] as $name => $value ) {
						header( $name . ': ' . $value );
					}
					echo (string) $GLOBALS['__x402_pay_response']['body']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- JSON body generated by wp_json_encode.
					exit;
				}
			}
		);
