...
use X402Press\Connectors\Coinbase\Registrar as CoinbaseRegistrar;
...
use X402Press\Payment\Providers\EvmWallet\Provider as EvmWalletProvider;
use X402Press\Services\AllPostsModeNoticeEmitter;
...
final class Plugin {
	 ...
	 */
	private const FACILITATOR_AUTOPICKED_OPTION = 'x402press_facilitator_autopicked';
	...
	public static function boot(): void {
		...
		add_filter(
			'x402press_facilitator_for_connector',
		...
		add_action(
			...
			static function () use ( $controller ): void {
				...
				foreach ( $GLOBALS['__x402press_response']['success_headers'] ?? array() as $line ) {
					header( (string) $line, false );
				}

				if ( ! empty( $GLOBALS['__x402press_response']['exited'] ) ) {
					foreach ( $GLOBALS['__x402press_response']['headers'] as $name => $value ) {
						header( $name . ': ' . $value );
					}
					echo (string) $GLOBALS['__x402press_response']['body']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- JSON body generated by wp_json_encode.
					exit;
				}
			}
		);
